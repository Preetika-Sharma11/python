...
class MultilingualATM:      
    ...
    def user_login(self, entered_pin, user_id, target_language='en'):
        if not self.logged_in:
            actual_pin = '1234'  # Change this to your actual PIN
    ...
    def deposit(self, amount, target_language='en'):
        if self.logged_in:
           ...
           # Update the user's balance in the users list
    ...
    def withdraw(self, amount, target_language='en'):
        if self.logged_in:
            if amount <= self.balance:
               self.balance -= amount
               self.transaction_history.append(f"Withdrew Rs{amount}. New balance: Rs{self.balance}")
               # Update the user's balance in the users list
               user = next((user for user in self.users if user['user_id'] == self.user_id), None)
               if user:
                  user['balance'] -= amount
               return f"{self.translate_input(f'Withdrew Rs{amount}. New balance: Rs{self.balance}', target_language)}"
            else:
    ...
    def translate_input(self, input_text, target_language='en'):
        if target_language == 'hi':
            ...
            return translation_dict.get(input_text, input_text)
...
def atm_simulator():
    correct_password = '1234'  # Replace with your actual correct password
    entered_password = input("Enter your password: ")

    if correct_password != entered_password:
        send_email("alert", "someone is trying to enter your password!!!!")
        frequency = 1000  # Set the frequency (Hz)
        duration = 1000  # Set the duration (ms)
